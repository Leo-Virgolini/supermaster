...
public final class MlDescripcionSugeridaBuilder {
    ...
    public static String construir(Producto p) {
        ...
        if (p.getSku() != null && !p.getSku().isBlank())
            sb.append("\nSKU: ").append(p.getSku().trim()).append("\n");  // línea en blanco entre la última característica y el SKU
        return sb.toString();
    }
    ...
    private static String aptos(Producto p) {
        ...
        nombres.sort(Collator.getInstance(Locale.of("es", "AR")));  // orden alfabético estable (getProductosApto() es un Set)
