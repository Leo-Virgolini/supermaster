-- Catálogo de sectores de depósito de Dux. El PK es autoincremental (interno).
...
--
-- ATENCIÓN: si la tabla `sectores_deposito` ya fue creada con la versión anterior
...
--   DROP TABLE IF EXISTS supermaster.sectores_deposito;
-- Es seguro hacerlo porque es un catálogo fijo y todavía no hay FKs de
-- producción que dependan de sus filas.
CREATE TABLE IF NOT EXISTS supermaster.sectores_deposito (
    id_sector_deposito INT         AUTO_INCREMENT PRIMARY KEY,
    codigo             VARCHAR(20) NOT NULL,
    id_dux             INT         NULL,
    CONSTRAINT uk_sectores_deposito_codigo UNIQUE (codigo)
);

INSERT INTO supermaster.sectores_deposito (codigo, id_dux) VALUES
