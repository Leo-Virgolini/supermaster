...
import { SectorDepositoDTO } from "./types";
...
export const getColumns = (canEdit = true): ColumnDef<SectorDepositoDTO>[] => [
    ...
    // --- COLUMNA 3: CÓDIGO (Editable estilo Excel, obligatorio) ---
    {
        accessorKey: "codigo",
        header: "Código",
        meta: { editable: true },
    ...
    // --- COLUMNA 4: ID DUX (Editable numérico, nullable) ---
    {
        accessorKey: "idDux",
        header: "ID Dux",
        meta: { editable: true },
        cell: ({ getValue, row, column, table }) => {
            const initialValue = getValue() as number | null;
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <Link
                href={`/productos?sectorDepositoIds=${row.original.id}`}
                title="Ver productos de este sector de depósito"
